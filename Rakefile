...
task :watch do
  ...
  jekyll_pid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll build -d #{public_dir} -w")
...
task :preview do
  ...
  jekyll_pid = Process.spawn({"OCTOPRESS_ENV"=>"preview"}, "jekyll s -d #{public_dir} -P #{server_port}")
